...
import { CustomTable, GetMap, NoData, ParameterCard } from "../../../../../components";
...
const Overview = () => {
  ...
  const { getLastUpdatedSensorData, lastUpdatedSensorData, getDeviceAlerts } =
    useViewSiteContext();
  const { getAllAssignedNodesOfABM, allAssignedNodesOfABM } =
    useDeviceContext();

  useEffect(() => {
    const filters = { depth: "6", assignedBranchManager: id };
